...
CryptoScanner generates **CycloneDX 1.6** compliant Cryptography Bill of Materials (CBOM), compatible with **IBM CBOMKit** (CBOMkit-coeus viewer).

> **Important**: The `findings.json` file generated by default scans is CryptoScanner's internal format, NOT a CycloneDX CBOM. You must use the `--cbom` flag to generate a standards-compliant CBOM.

```bash
# Generate JSON CBOM (saved in ./src/cbom.json)
cargo run --release -- --path ./src --cbom --app-name "MyApp"

# Generate XML CBOM (saved in ./src/cbom.xml)
cargo run --release -- --path ./src --cbom --cbom-format xml --cbom-output cbom.xml --app-name "MyApp"

# Custom output filename (saved in ./src/crypto-report.json)
cargo run --release -- --path ./src --cbom --cbom-output crypto-report.json --app-name "MyApp"

# Generate CBOM for IBM CBOMKit import
cargo run --release -- --path /path/to/project --cbom --app-name "ProjectName"
```

> **Output behavior**: All result files (`findings.json` and CBOM) are always saved in the scanned project directory. The `--cbom-output` flag controls the filename only.

**CBOM Asset Types Generated:**
- **algorithm** — Detected cryptographic algorithms (AES, RSA, SHA-256, HMAC, ECDSA, etc.) with primitive type, mode, and key size
- **certificate** — Certificate files (.pem, .crt, .cer, .der) with format and extension metadata
- **related-crypto-material** — Keystores (.p12, .jks, .pfx), private keys (.key), and other crypto material

**Algorithm Extraction:**
- OpenSSL commands are parsed for algorithm and key size (e.g., `openssl genrsa 2048` → RSA-2048)
- Java imports are mapped to crypto primitives (e.g., `GCMParameterSpec` → AES-GCM, `Mac` → HMAC)
- Bouncy Castle operations are detected and mapped to appropriate algorithms
- False positives are filtered (URL paths, curl commands, YAML configs, Spring Security matchers)
...
- **CycloneDX 1.6 Compliance**: Generates CBOM documents compatible with IBM CBOMKit
- **Three-Phase Component Generation**: Processes keystore → library → secret findings separately
- **Intelligent Algorithm Extraction**: Parses code context to identify actual algorithms (not just library names)
- **Three Asset Types**: `algorithm`, `certificate`, `related-crypto-material`
- **False Positive Filtering**: Excludes URL paths, curl commands, YAML configs, and framework matchers
...
5. **Report Generation**: Results exported to JSON and optionally CBOM, always saved in the target project path
